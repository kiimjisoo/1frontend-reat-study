...
// state 초기값으로 원시값, 배열, 객체 등 모든 데이터 타입 사용 가능 - 빈문자 배열 0 가능_원시값은 나중에 복잡해짐
...
// 전역 state 만드는 방법
// useState()랑 비슷한 역할을 하는데 Redux 에서는 state하나를 Slice라고 부름
// createSlice() 함수: state이름, 초기값 설정, 액션 및 리듀서 함수를 만드는 것을 도와줌
// 인자값으로 name, initialState, reducers 속성을 갖는 객체를 넣음
export const counterSlice = createSlice({
  name: 'counter', // state 이름(추가로 action 이름을 만든느데도 쓰임)
  initialState , // state의 초기값 (위에서 객체 형태로 정의) (initialState:initialState 일부러 변수명 똑같이 함 같으면 생략 가능하기 때문에)
  ...
  reducers: {
    increment: (state) => { // 첫번째 파라미터: 현재 state를 받아옴
      ...
      // state에 바로 접근하지 않고 복사본에 저장해서 set함수에 넣어야 하는 걸 immer가 알아서 함
    ...
    incrementByAmount: (state, action) => { // 두번째 파라미터: action 객체 = {type: 발생한 액션의 타입, payload: 화물, 전달 데이터}
      console.log(action); // payload, type 찍힘
...
// 3) 수정을 원할 때 그 함수를 실행해달라고 store에 요청을 해야됨 => dispatch() 함수를 사용해 store에 액션을 보냄
