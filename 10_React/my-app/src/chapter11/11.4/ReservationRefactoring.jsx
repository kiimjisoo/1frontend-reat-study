...
function ReservationRefactoring(props) {
  ...
  // input에 name속성을 설정하고 이벤트가 발생했을 때 이 값을 참조하여 객체 접근
  const [inputs, setInputs] = useState({
    breakfast: false,
    numberOfGuests: 2
  });
  const { breakfast, numberOfGuests } = inputs; // ES6차 구조 분해 할당을 통해 갑 추출
  ...
  const handleInputChange = (e) => {
    ...
    // setInputs(inputs); // set함수를 써도 재렌더링이 발생 안 함(리액트가 객체가 바뀐것을 감지하지 못함)

    // 그 대신 새로운 객체를 만들어서 새로운 객체에 변화를 주고 이를 상태로 사용해야함
