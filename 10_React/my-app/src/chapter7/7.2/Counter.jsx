...
function Counter() {
  ...
  // Quiz: 글자색 바꾸기, 버튼내용 바꾸기
  const [color, setColor] = useState('black');
  const [clicked, setClick] = useState('신청');
  ...
  return (
    ...
    <div>
      ...
      {/* 버튼 내용 바꾸기 */}
      <button type="button" onClick={() => setClick('마감')}>{clicked}</button>
      {/* 삼항연산자 사용시 */}
      <button type="button" onClick={() => setFinish(true)} disabled={finish}>{finish ? '마감' : '신청'}</button>
