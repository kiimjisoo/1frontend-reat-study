...
function ApiRequest(props) {
  ...
  const handleRequestByIdComment = async (id) => {
    try {
      const resp = await axios.get(`https://jsonplaceholder.typicode.com/comments/${id}`)
      setData(resp.data)
    } catch(error) {
      console.error(error);
    }
  ...
  return (
    <>
      <div>
        ...
        <button type='button' onClick={() => handleRequestByIdAsync(20)}>async불러오기</button>
        <button type='button' onClick={() => handleRequestByIdComment(20)}>async불러오기2</button>
      </div>
      {data && // 데이터 들어왔을때
        <>
