      ...
      <div className="inner">
        ...
        {posts.map((post, index) => {
            ...
            >
              ...
              <div className="toolbar">
                ...
                <span onClick={(e) => {
                  ...
                  e.stopPropagation(); // 상위 요소로 퍼지는 이벤트 버블링 막음
                ...
                <span style={{ fontSize: 27 }} 
                  onClick={(e) => {
                    // div를 직접 제거하는것x
                    // state에서 제거하면 알아서 자동으로 렌더링ㅇ
                    e.stopPropagation();

                    // const copyAllPost = [...posts];
                    // copyAllPost.splice(index, 1 );
                    // setPosts(copyAllPost);

                    // 또는 배열의 filter() 함수 사용
                    const filteredPost = posts.filter((value, idx) => {
                      return idx !== index;
                    });
          ...
          {/* 
            ...
            2. 등록 버튼 클릭시 posts 상태에 맨 앞에 새로운 데이터 추가
            */}
          <input type='text' value={newPost} onChange={handleNameChange} />
          {/* <input type='text' value={inputValue} onChange={e => setInputValue(e.target.value)} /> */}
          <button onClick={() => {
            // div하나를 새로 생성x
            // posts state에 요소 하나 추가하면 자동으로 렌더링ㅇ
            const newCopyPost = [...posts];
            newCopyPost[newPost] = newCopyPost.unshift(newPost);
            setPosts(newCopyPost);

            // copyPosts.unshift(inputValue);
            // setPosts(copyPosts);
            // const copyPosts = [inputValue, ...posts];
            // setInputValue('');
            setNewPost(''); // input 초기화
