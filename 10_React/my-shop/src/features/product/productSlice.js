...
export const getMoreProductAsync = createAsyncThunk(
  ...
  async () => { // 두번째 인자값: action이 발생했을 때 실행할 비동기 작업(api 요청 같은)
    ...
    return result; // 값을 반환하면 'fulfilled' 상태로 바뀌고 action.payload에 담겨 리듀서 함수로 전달됨 // return 꼭 해야함
...
const productSlice = createSlice({
  ...
  // extraReducers로 비동기 작업을 할 때는 액션 생성 함수를 자동으로 만들지 못함 (그래서 위에서 만듦)
  extraReducers: (builder) => {
    builder
      .addCase(getMoreProductAsync.pending, (state) => { // pending 상태 일 때 동작할 리듀서 함수
        state.status = 'loading';
      })
      .addCase(getMoreProductAsync.fulfilled, (state, action) => { // fulfilled 상태 일 때 동작할 리듀서 함수
        state.status = 'idle'; // complete, success 등
        state.productList.push(...action.payload);
      })
      .addCase(getMoreProductAsync.rejected, (state) => { // rejected 상태 일 때 동작할 리듀서 함수
        state.status = 'fail';
      });
  }
